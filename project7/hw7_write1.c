...
const char *string_to_write = "hw6-1, 110511010";   // Task0: Change this string with your own student ID.

int main() {
    // 1. Create and Open your own shared file
    int fd = open("shared_file1", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);               // Task1: Complete open() function.
    if (fd == -1) {
        perror("open");
        return 1;
    }
