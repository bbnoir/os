...
const char *string_to_write = "hw6-2, 110511010";   // Same as hw6_write1.c, but this one is "hw6-2".

int main() {
    // 1. Create and Open your own shared file
    int fd = open("shared_file2", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);               // Same as hw6_write1.c.
    if (fd == -1) {
        perror("open");
        return 1;
    }
